...
本文档说明 Tim2Tox 当前的 Bootstrap 节点加载、连接建立和轮询机制，并补充 Flutter Echo Client 在启动、设置页和局域网 Bootstrap 场景下的接入方式。

## 1. 范围

这条链路横跨两层：

- `flutter_echo_client`：负责选择和保存当前 Bootstrap 节点、决定启动顺序、提供局域网 Bootstrap UI。
...
Flutter Echo Client 在 `_StartupGate._decide()` 中会先检查 Bootstrap 模式：
...
局域网 Bootstrap 模式是 Flutter Echo Client 的配置层概念，不是 Tim2Tox 内部协议分支。当前实现里：
...
- [../../flutter_echo_client/doc/ACCOUNT_AND_SESSION.md](../../flutter_echo_client/doc/ACCOUNT_AND_SESSION.md)
- [../../flutter_echo_client/doc/IMPLEMENTATION_DETAILS.md](../../flutter_echo_client/doc/IMPLEMENTATION_DETAILS.md)
