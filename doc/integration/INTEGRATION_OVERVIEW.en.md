...
| **Best for** | **Minimal code change**, only swapping the backend to Tox; quick validation or clients that prioritize “compatible with native calls.” | When you need **history, polling, custom callbacks, Bootstrap config, call bridge** implemented on the Dart side; full product clients (e.g. toxee). |
...
| Match existing toxee setup | Hybrid: binary replacement + Tim2ToxSdkPlatform; history, polling, Bootstrap, etc. go through Platform. |
...
**Reference implementation**: [toxee](../../../) (hybrid setup, interface implementations, Bootstrap, history).
...
- [ARCHITECTURE.en.md](../architecture/ARCHITECTURE.en.md) §10 — Relationship with clients (e.g. toxee)
