...
| **适用场景** | 希望**最少改业务代码**、仅替换后端为 Tox；适合快速验证或以“兼容原生调用”为主的客户端。 | 需要**历史消息、轮询、自定义回调、Bootstrap 配置、通话桥**等由 Dart 侧统一实现时；适合完整产品化客户端（如 toxee）。 |
...
| 与现有 toxee 一致 | 混合：二进制替换 + Tim2ToxSdkPlatform，历史/轮询/Bootstrap 等走 Platform。 |
...
**参考实现**：[toxee](../../../)（混合架构、接口实现、Bootstrap 与历史等）。
...
- [ARCHITECTURE.md](../architecture/ARCHITECTURE.md) §10 — 与客户端（如 toxee）的关系
