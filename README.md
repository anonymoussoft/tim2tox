...
- 相关 Flutter Echo Client 文档：[../flutter_echo_client/doc/README.md](../flutter_echo_client/doc/README.md) / [../flutter_echo_client/doc/README.en.md](../flutter_echo_client/doc/README.en.md)
...
Tim2Tox 同时支持二进制替换路径和 Platform/FfiChatService 路径；当前 Flutter Echo Client 采用的是**混合架构**。
...
对于生产环境应用（如 `flutter_echo_client`），直接使用 `TIMManager.instance` 即可，它会自动使用默认实例：
...
参考 `../flutter_echo_client/` 目录，这是一个完整的 Flutter 客户端示例，展示了如何使用 tim2tox framework。
